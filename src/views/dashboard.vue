            ...
            <el-col :span="12">
                <el-card style="width: 100%">
                    ...
                    <div class="chart-content">
                        ...
                        <div ref="consultationChartRef" style="width: 100%; height: 350px;">
                        </div>
                    </div>
                </el-card>
            </el-col>
...
const aiData = ref({
  ...
  emotionTrend: [], // 情绪趋势数据默认是空数组
  emotionDistribution: [] // 情绪分布数据默认是空数组
})
...
const option = {
  ...
  yAxis: [
    ...
    // 右轴：参与用户数，数值小
    {
      ...
      max: 40,
      axisLabel: { color: '#636e72' },
      axisLine: { lineStyle: { color: '#fdcb6e' } },
      splitLine: { show: false }
    }
  ],
  series: [
    // 会话数量 绑定 左轴（yAxisIndex: 0）
    {
      name: '会话数量',
      type: 'bar',
      yAxisIndex: 0,
...
onMounted(() => {
    getAnalyticsOverview().then(res => {
        Object.assign(aiData.value, res)
        console.log('数据分析概览:', res)
        //数据加载完成后初始化图表
        initChart()
        initUserActivityChart()
    }).catch(err => {
        console.error('获取数据分析概览失败:', err)
    }).finally(() => {
        // 无论成功还是失败，都关闭加载状态
        loading.value = false
    })
